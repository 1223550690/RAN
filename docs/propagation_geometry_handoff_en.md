...
This handoff documents Zhiqian's propagation geometry module for the RAN MVP.
...
- `coordinate_view`: optional `CoordinateCalibrationView`; currently not required.
...
2. Compute map-unit distance. Meter fields remain `None` unless a calibration view is provided.
...
- `distance_2d_m`: `None` until coordinate calibration is supplied.
...
- `outdoor_distance_m` and `indoor_distance_m`: `None` until calibration is supplied.
...
5. Coordinate with Yuhan before using meter-based distances or height-based 3D distance.

## Known Limitations

- No coordinate calibration is implemented here.
- Meter distances are `None` without a supplied calibration view.
- No formal pytest file has been added yet; the debug script is the current smoke test.
...
- Does this change any shared interface? Expected answer: no.
- Does this delete or rename any field? Expected answer: no.
- Does this modify `channel.py` or `ChannelState`? Expected answer: no.
- Are meter fields clearly documented as optional and currently `None`?
