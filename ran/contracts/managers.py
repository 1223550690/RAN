    ...
    def updateManagers(self):
        for connection in self.transportLayer.connectionTable:
            if self.connections[connection] == "CONNECTED":
                ...
                for i in range(startingIndex, math.floor(byteLength/MTU)+startingIndex):
                    if (i+1)*8*MTU < len(connectdata.dataToLeave):
...
host1.send(source_port=144, targetPort=187, targetProtocol="TCP", data="10011101", targetIp="10.30.3.40", srcIp="10.20.2.20")
...
while(len(host1.messageBuffer) or len(host2.messageBuffer)):
    for message in host1.messageBuffer:
        ...
        host1.messageBuffer.remove(message)
    ...
    for message in host2.messageBuffer:
            ...
            print(transheader, len(data))
            host2.messageBuffer.remove(message)
